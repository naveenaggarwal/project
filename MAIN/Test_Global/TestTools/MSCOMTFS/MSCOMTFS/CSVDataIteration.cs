    ...
    public class CSVDataIteration
    {
        ...
        public string this[string name]
        {
            get
            {
                ...
                {
                    throw new KeyNotFoundException(string.Format("The parameter '@{0}' was not found in the CSVDataIteration. Make sure Test Case parameter and what's been requested are the same.", name));
                }
            }
        }
        ...
        internal CSVDataIteration(Dictionary<string, string> values)
        {
            ...
            foreach (string oldValue in descTestDataDictionary) //for each entry in the ReplacementDictionary
            {
                ...
                foreach (string key in data.Keys) //for each parameter in the provided iteration (valuesToReplace)
                {
                    ...
                    if (current.Contains(oldValue))
                    {
                        ...
                        if (!wasReplaced)
                        {
                            MSCOM.Test.Tools.TestAgent.LogToTestResult(string.Format("Note - Value from CSV Data Iteration '{0}' matched originalValue key '{1}' (found in CSVReplacementValues.csv)" +
                        ...
                        else
                        {
                            MSCOM.Test.Tools.TestAgent.LogToTestResult(string.Format("Warning - While replacing Itration Parameter's data, a key was found that could lead to unexpected " +
                                "or unwanted replacements of data. \nSpecifically, value from CSV Data Iteration '{0}' matched originalValue key '{1}' (found in CSVReplacementValues.csv)" +
